...
namespace openworld
{
    ...
    public:
        ...
        template <typename T>
        constexpr memory_region(T* data, size_type size) :
            m_element_size(sizeof(T)),
            m_element_count(size),
            m_data(reinterpret_cast<pointer>(data))
        {}

        constexpr memory_region(pointer data, size_type size_bytes, size_type element_size) :
        ...
        template <typename T>
        constexpr memory_region(std::vector<T>& data) :
            m_element_size(sizeof(T)),
            m_element_count(data.size()),
            m_data(reinterpret_cast<pointer>(data.data()))
        {}

        constexpr memory_region(const memory_region& other) = default;

        constexpr memory_region& operator=(const memory_region& other) = default;
        ...
        constexpr size_type size_element() const
        {
